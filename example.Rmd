...
We will consider an illness-death cost-effectiveness model in which patients can transition between three health states: healthy, sick, and death. The economic model will be analyzed using the `R` programming language. In this analysis, we will use five `R` packages. The [diagram](https://cran.r-project.org/web/packages/diagram/index.html) package will be used to visualize the illness-death model. [flexsurv](https://cran.r-project.org/web/packages/flexsurv/index.html) and mstate are used to estimate a multi-state statistical model needed to parameterize the economic model. [hesim](https://innovationvalueinitiative.github.io/hesim/) is then used to simulate the economic model conditional on the estimated parameters. Finally, [BCEA](https://sites.google.com/a/statistica.it/gianluca/bcea) is used to analyze the output (i.e., quality-adjusted life-years (QALYs) and costs) produced by `hesim`.
...
cox_fits <- survival::coxph(Surv(years, status) ~ strata(trans) + strategy_id, data = mstate_data) 
...
for (i in 1:length(ggamma_fits)){
  ggamma_fits[[i]] <- flexsurv::flexsurvreg(Surv(years, status) ~ strategy_id, 
                                            data = mstate_data, 
                                            subset = (trans == i) , 
                                            dist = "gengamma") 
}
...
```{r medical_costs}
medcosts_mean <- c(800, 1500)
medcosts_se <- c(100, 150)
gamma_params <- mom_gamma(mean = medcosts_mean, sd = medcosts_se)
medcost_means_posterior <- matrix(rgamma(n_samples * 2, shape = gamma_params$shape,
...
```{r utility}
utility_mean <- c(0.90, 0.55)
utility_se <- c(.1, .2)
beta_params <- mom_beta(mean = utility_mean, sd = utility_se)
utility_means_posterior <- matrix(rbeta(n_samples * 2, shape1 = beta_params$shape1,
                                  shape2 = beta_params$shape2),
                                  nrow = n_samples, byrow = TRUE)
utility_means <- stateval_means(values = utility_means_posterior,
                                strategy_id = c(1, 2),
                                patient_id = seq(1, 1000))
```

# Simulation
Given parameter estimates for the disease model, costs, and utility, the cost-effectiveness model can be simulated. We can do this using the `hesim` package. We must first set up our model by specifying the treatment strategies, the patients to model, and the number of health states. Since we fit a clock-reset multi-state model, we can only estimate health state occupancy using an individual patient simulation. Since we are ultimately interested in simulated mean costs and QALYs (across patients), we simualte the model for 1,000 patients to ensure that our simulated means are stable.

```{r sim_setup}
strategies <- data.frame(strategy_id = c(1, 2))
patients <- data.frame(patient_id = seq(1, 1000))
states <- data.frame(state_id = seq(1, 3),
                     state_name = c("Healthy", "Sick", "Death"))
...
We can then setup the model for disease progression which simulates health state transitions by treatment strategy and patient. 

```{r transmod}
trans_data <- expand_hesim_data(hesim_dat, 
                                by = c("strategies", "patients"))
ggamma_fits <- hesim::flexsurvreg_list(ggamma_fits)
transmod <- hesim::create_IndivCtstmTrans(ggamma_fits, data = trans_data, trans_mat = tmat,
                                          n = n_samples)
```

We can also setup models for costs and utility used our estimated mean values.

```{r medcost_mod}
# Medical cost model
medcost_means <- stateval_means(values = medcost_means_posterior,
                               strategy_id = c(1, 2),
                               patient_id = seq(1, 1000))
medcostmod <- create_StateVals(medcost_means)

# Drug cost model
drugcost_array <- array(NA, dim = c(n_samples, 2, 2))
drugcost_array[, , 1] <- drugcost1
drugcost_array[, , 2] <- drugcost2
...
# Decision analysis
Summaries of the clinical benefit (e.g., QALYs) and costs of the two treatment strategies simulated using the cost-effectiveness model are stored in the matrices `qalys_mat` and `costs_mat`. Each row is a simulated parameter set from the PSA and each column is a unique treatment strategy. 
...
Given this output, we can use the function `bcea` to summarize the results of the cost-effectiveness model and compute suitable measures of "cost-effectiveness". 
