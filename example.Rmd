...
# Simulation
Given parameter estimates for the health state transitions, costs, and utility, the economic model can be simulated. We can do this using the `hesim` package. We must first set up our model by specifying the treatment strategies, the patients to model, and the number of health states. Since we fit a clock-reset multi-state model, we can only estimate health state occupancy using an individual patient simulation. Since we are ultimately interested in simulated mean costs and QALYs (across patients), we simulate the model for 1,000 patients to ensure that our simulated means are stable.
...
We can also setup models for costs and utility using simulated posterior distributions for the mean values. In each case, we specify that costs and utility will be predicted for 2 treatment strategies and 1,000 patients. Medical costs and utilities vary across health states (but not patients or treatment strategies) while drug costs vary across treatment strategies (but not patient or health states).
...
Once (statistical) models for the health state transitions, costs, and utility have been specified, the economic model can be simulated. We used an individual-level continuous time state transition model (CTSTM) to simulate disease progression, often referred to as a semi-Markov multi-state model. 
